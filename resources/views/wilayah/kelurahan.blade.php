...
@section('page-title', 'Wilayah - Kelurahan')
@section('breadcrumb')
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-hover table-striped align-middle mb-0">
                <thead class="table-light"><tr><th width="50">No</th><th>Kode Wilayah</th><th>Nama Kelurahan/Desa</th><th>Kecamatan Induk</th></tr></thead>
                <tbody>
                    @forelse($kelurahans as $i => $item)
                    <tr>
                        <td>{{ $kelurahans->firstItem() + $i }}</td>
                        <td><span class="badge bg-secondary font-monospace">{{ $item->kode }}</span></td>
                        <td class="fw-bold">{{ $item->nama }}</td>
                        <td>{{ $item->kecamatan->nama ?? '-' }}</td>
                    </tr>
                    @empty
                    <tr><td colspan="4" class="text-center text-muted py-5"><i class="fas fa-folder-open fa-2x mb-2 opacity-50"></i><br>Belum ada data.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
